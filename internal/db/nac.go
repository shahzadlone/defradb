...
func (db *DB) initializeNodeACP(ctx context.Context, txn datastore.Txn) error {
	...
	if !wasSetupBefore {
		...
		if err := db.cleanRegisterInternalNACPolicyAndGateNode(ctx); err != nil {
			return err
		}
...
// cleanRegisterInternalNACPolicyAndGateNode will register policy and then register the node with the
// node acp system, if anything goes wrong, it will leave the node acp in the clean state.
...
func (db *DB) cleanRegisterInternalNACPolicyAndGateNode(ctx context.Context) error {
	iden := identity.FromContext(ctx)
	if !iden.HasValue() {
