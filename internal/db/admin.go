...
// ReEnableAdminACP will re-enable admin acp that was temporarily disabled (and configured). This will
...
func (db *DB) ReEnableAdminACP(ctx context.Context) error {
...
// DisableAdminACP will disable admin acp for the users temporarily. This will keep the current admin acp
...
// Returns an [client.ErrNotAuthorizedToPerformOperation] error if the requesting identity is not
// authorized to perform this operation.
func (db *DB) DisableAdminACP(ctx context.Context) error {
	ctx, span := tracer.Start(ctx)
	defer span.End()
...
// PurgeAdminACP will reset/purge the entire admin acp state. This means that all relationships that were
...
func (db *DB) PurgeAdminACP(ctx context.Context) error {
	ctx, span := tracer.Start(ctx)
	defer span.End()

	if err := db.checkAdminAccess(ctx, acpTypes.AdminAACPurgePerm); err != nil {
		return err
	}
	...
	// TODO-ACP-ADMIN: Check if need to close or not, keep in mind we need admin acp on all times
	...
	err = db.adminInfo.AdminACP.Close()
	if err != nil {
		return err
	}
...
func (db *DB) checkAdminAccess(
	...
	if permissionNeeded.IsForAACOperation() && db.adminInfo.AdminDesc.IsClean && permissionNeeded != acpTypes.AdminAACStatusPerm {
		return ErrAACIsNotConfigured
	}
	...
	return CheckAccessOfNodeOperationWithAdminACP(
		ctx,
		ident,
		db.adminInfo,
		permissionNeeded,
...
// CheckAccessOfNodeOperationWithAdminACP returns an [client.ErrNotAuthorizedToPerformOperation]
...
func CheckAccessOfNodeOperationWithAdminACP(
	ctx context.Context,
...
func (db *DB) addActorRelationshipWithAAC(
	ctx context.Context,
	relation string,
	targetActor string,
...
func (db *DB) deleteActorRelationshipWithAAC(
	ctx context.Context,
	relation string,
	targetActor string,
...
func (db *DB) AddActorRelationshipWithAAC(
	...
	return db.addActorRelationshipWithAAC(ctx, relation, targetActor)
}
...
func (db *DB) DeleteActorRelationshipWithAAC(
	...
	return db.deleteActorRelationshipWithAAC(ctx, relation, targetActor)
}
...
func (db *DB) AdminACPStatus(ctx context.Context) (client.StatusAACResult, error) {
	ctx, span := tracer.Start(ctx)
	defer span.End()
