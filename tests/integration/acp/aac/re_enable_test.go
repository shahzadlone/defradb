...
func TestAAC_ReEnableWithNoIdentityWhenTemporarilyDisabled_Error(t *testing.T) {
	test := testUtils.TestCase{
		...
		Actions: []any{
			...
			testUtils.GetAdminACPStatus{ // Still disabled
...
func TestAAC_ReEnableWithValidIdentityWhenTemporarilyDisabled_AACReEnabled(t *testing.T) {
	test := testUtils.TestCase{
		...
		Actions: []any{
			...
			testUtils.GetAdminACPStatus{ // AAC was successfully re-enabled so this won't work
				ExpectedError: "not authorized to perform operation",
			},

			testUtils.GetAdminACPStatus{ // This works and shows that aac is enabled.
				Identity:             testUtils.ClientIdentity(1),
...
func TestAAC_ReEnableWithNoIdentityWhenAlreadyEnabled_Error(t *testing.T) {
	test := testUtils.TestCase{
		...
		Actions: []any{
			...
			testUtils.ReEnableAdminACP{ // AAC is already enabled before.
				ExpectedError: "admin acp is already enabled",
			},

			testUtils.GetAdminACPStatus{
				Identity:             testUtils.ClientIdentity(1),
				ExpectedIsEnabled:    true,
				ExpectedIsConfigured: true,
			},
...
func TestAAC_ReEnableSuccessfullyThenRestartWithNoArgs_RemainsReEnabled(t *testing.T) {
	test := testUtils.TestCase{
		...
		Actions: []any{
			...
			testUtils.GetAdminACPStatus{ // Remains enabled even after restart.
				Identity:             testUtils.ClientIdentity(1),
				ExpectedIsEnabled:    true,
				ExpectedIsConfigured: true,
			},
...
func TestAAC_ReEnableTemporarilyDisabledAACAfterRestart_ReEnabledSuccessfully(t *testing.T) {
	test := testUtils.TestCase{
		...
		Actions: []any{
			...
			testUtils.GetAdminACPStatus{ // Remains disabled ofcourse even after restart.
				ExpectedIsEnabled:    false,
				ExpectedIsConfigured: true,
			},

			testUtils.ReEnableAdminACP{ // Successfully re-enabled again, even after restart.
				Identity: testUtils.ClientIdentity(1),
			},

			testUtils.GetAdminACPStatus{ // This should then not work.
				ExpectedError: "not authorized to perform operation",
			},
