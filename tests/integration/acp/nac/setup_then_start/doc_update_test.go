...
func TestNAC_GatesDocUpdatePreSetup_AllowIfAuthorizedElseError(t *testing.T) {
	test := testUtils.TestCase{
		Description: "admin acp correctly gates doc update operation (setup before nac), allow if authorized, otherwise error",
...
func TestNAC_GatesDocUpdateWithFilterPreSetup_AllowIfAuthorizedElseError(t *testing.T) {
	test := testUtils.TestCase{
		Description: "admin acp correctly gates doc update with filter operation (setup before nac), allow if authorized, otherwise error",
		SupportedDatabaseTypes: immutable.Some(
			[]testUtils.DatabaseType{
				// This test only supports file type databases since the setup steps will be done before
				// the node is re-started with nac enabled (if it's in-memory it will loose setup state).
