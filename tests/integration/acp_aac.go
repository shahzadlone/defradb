...
// ReEnableAdminACP will attempt to re-enable a temporarily disabled Admin ACP system.
type ReEnableAdminACP struct {
...
// reEnableAdminACP will attempt to re-enable the admin access control system.
func reEnableAdminACP(
	s *state,
	action ReEnableAdminACP,
) {
	nodeIDs, nodes := getNodesWithIDs(action.NodeID, s.nodes)
	for index, node := range nodes {
		nodeID := nodeIDs[index]
		ctx := getContextWithIdentity(s.ctx, s, action.Identity, nodeID)
		err := node.ReEnableAdminACP(ctx)
...
// DisableAdminACP will attempt to temporarily disable DefraDB's Admin ACP system.
type DisableAdminACP struct {
...
// disableAdminACP will attempt to start the admin access control system.
func disableAdminACP(
	s *state,
	action DisableAdminACP,
) {
	nodeIDs, nodes := getNodesWithIDs(action.NodeID, s.nodes)
	for index, node := range nodes {
		nodeID := nodeIDs[index]
		ctx := getContextWithIdentity(s.ctx, s, action.Identity, nodeID)
		err := node.DisableAdminACP(ctx)
...
// AddActorRelationshipWithAAC will attempt to create a new relationship for node access with an actor.
type AddActorRelationshipWithAAC struct {
...
func addActorRelationshipWithAAC(
	s *state,
	action AddActorRelationshipWithAAC,
) {
	nodeIDs, nodes := getNodesWithIDs(action.NodeID, s.nodes)
	for index, node := range nodes {
		nodeID := nodeIDs[index]

		addActorRelationshipResult, err := node.AddActorRelationshipWithAAC(
...
// DeleteActorRelationshipWithAAC will attempt to delete a relationship for node access with an actor.
type DeleteActorRelationshipWithAAC struct {
...
func deleteActorRelationshipWithAAC(
	s *state,
	action DeleteActorRelationshipWithAAC,
) {
	nodeIDs, nodes := getNodesWithIDs(action.NodeID, s.nodes)
	for index, node := range nodes {
		nodeID := nodeIDs[index]

		deleteActorRelationshipResult, err := node.DeleteActorRelationshipWithAAC(
			getContextWithIdentity(s.ctx, s, action.RequestorIdentity, nodeID),
			action.Relation,
			getIdentityDID(s, action.TargetIdentity),
		)
...
// GetAdminACPStatus returns the current status of the target node(s) if all goes well,
// otherwise returns an error.
type GetAdminACPStatus struct {
...
func getAdminACPStatus(
	s *state,
	action GetAdminACPStatus,
) {
	nodeIDs, nodes := getNodesWithIDs(action.NodeID, s.nodes)
	for index, node := range nodes {
		nodeID := nodeIDs[index]

		statusAACResult, err := node.AdminACPStatus(
